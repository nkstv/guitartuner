(() => {
  ...
  async function startMic(){
    if(!navigator.mediaDevices?.getUserMedia){
      els.tuningMessage.textContent = "Your browser does not support microphone access.";
      return;
    }
    try{
      const ctx = ensureAudio();
      ...
      micRunning = true;
      els.micButton.innerHTML = '<span aria-hidden="true">■</span> Stop microphone';
      els.micStatusText.textContent = 'Microphone active';
      document.querySelector('.mic-dot')?.classList.add('is-live');
      els.tuningMessage.textContent = 'Play one string at a time.';
      analysePitch();
    } catch(e){
      els.tuningMessage.textContent = "Allow microphone access in your browser to use automatic tuning.";
    }
  }
  ...
  function stopMic(){
    ...
    analyser = null;
    micSource = null;
    els.micButton.innerHTML = '<span aria-hidden="true">●</span> Enable microphone';
    els.micStatusText.textContent = 'Microphone off';
    document.querySelector('.mic-dot')?.classList.remove('is-live');
    els.detectedNote.textContent = '—';
    els.detectedHz.textContent = 'Play a string';
    els.meterNeedle.style.left = '50%';
    els.tuningMessage.textContent = 'Turn on the microphone to start.';
  }
  ...
  function analysePitch(){
    ...
    if(freq>40 && freq<1200){
      ...
      if(Math.abs(rawCents)<=4) els.tuningMessage.textContent = 'In tune — perfect.';
      else if(rawCents<0) els.tuningMessage.textContent = 'Too low — tighten the string slightly.';
      else els.tuningMessage.textContent = 'Too high — loosen the string slightly.';
    }
